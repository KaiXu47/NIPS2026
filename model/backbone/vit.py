...
class Block(nn.Module):
    ...
    def forward(self, x, attn_bias=None):
        o, weights = self.attn(self.norm1(x), attn_bias=attn_bias)
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 drop_path_rate=0., hybrid_backbone=None, norm_layer=nn.LayerNorm, vis=False, aux_layer=-3, step=0):
        super().__init__()
...
@register_model
def vit_base_patch16_224(pretrained=False, in_chans=3, **kwargs):
    model = VisionTransformer(
        patch_size=16, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6),
        in_chans=in_chans,               # 关键：把 in_chans 传进去
        **kwargs
    )
